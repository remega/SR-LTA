function pred_map = grad_prediction(my_image,x_center, y_center)

% Function to filter a foveated image using the center-surround of gradients
...
load all_gabors_gradient_params_fft_results %Load the gradient filters
...
for gabs_ind = 1:size(final_gabors,3)
    ...
    gab_fft = fft2(use_gabor, FFT_SIZE(1), FFT_SIZE(2));
    gab_size = size(gab_fft);
    temp_filtered = abs(ifft2( image_fft .* gab_fft));
    ...
    temp_valid = temp_valid./sqrt(mean(temp_valid(:).^2)); %Normalize to unit energy
